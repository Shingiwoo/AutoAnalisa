...
import { createChart, ColorType, CandlestickData, LineWidth, LineStyle } from 'lightweight-charts'
...
export default function ChartOHLCV({
  data,
  overlays,
  className,
  mode, // 'scalping' | 'swing' (opsional)
}: {
  data: Row[],
  overlays?: {
    sr?: number[],
    tp?: number[],
    invalid?: InvalidMulti,
    entries?: number[],
    fvg?: FVG[],
    zones?: Zone[],
    ghost?: GhostOverlay,
    liq?: number,
    funding?: FundingWin[],
    llm?: { lines?: LLMLine[], zones?: LLMZone[], markers?: LLMMarker[], mode?: 'scalping' | 'swing' } | null
  },
  className?: string,
  mode?: 'scalping' | 'swing'
}) {
  ...
  useEffect(() => {
    ...
    // Warna TP mengikuti mode: scalping (hijau) vs swing (oranye).
    const _llmMode = (mode || (llmOverlay as any)?.mode || 'scalping') as ('scalping'|'swing')
    const TP_COLOR = _llmMode === 'swing' ? '#f59e0b' : '#16a34a'
    ...
    // Base TP (jika ditampilkan) ikut warna mode juga agar konsisten visual
    if (overlays?.tp){
      for (const t of overlays.tp){
        series.createPriceLine({ price: t, color: TP_COLOR, lineWidth: 2, lineStyle: 0, title: 'TP' })
      }
     }
    if (overlays?.sr) {
    ...
    // Ghost overlay (V2 preview) dashed
    if (overlays?.ghost) {
      const g = overlays.ghost
      if (typeof g.invalid === 'number') series.createPriceLine({ price: g.invalid, color: '#0ea5e9', lineWidth: 2, lineStyle: 2, title: 'V2 SL' })
      if (Array.isArray(g.tp)) for (const t of g.tp) { series.createPriceLine({ price: t, color: '#22c55e', lineWidth: 1, lineStyle: 2, title: 'V2 TP' }) }
      if (Array.isArray(g.entries)) for (const e of g.entries) { series.createPriceLine({ price: e, color: '#06b6d4', lineWidth: 1, lineStyle: 2, title: 'V2 Entry' }) }
    }
    if (llmOverlay && Array.isArray(llmOverlay.lines)) {
      for (const ln of llmOverlay.lines) {
        ...
        let style: LineStyle = 0;

        if (label.includes('SL')) {
          color = '#ff4d4f';
          width = 2;
          style = 0;
