...
export default function PlanCard({plan, onUpdate, llmEnabled, llmRemaining, onAfterVerify}:{plan:any,onUpdate:()=>void, llmEnabled?:boolean, llmRemaining?:number, onAfterVerify?:()=>void}){
  ...
  useEffect(()=>{ (async()=>{
    try{ setLoading(true); const {data}=await api.get('ohlcv', { params:{ symbol:plan.symbol, tf, limit:200, market: 'spot' } }); setOhlcv(data) }catch{} finally{ setLoading(false) }
  })() },[tf, plan.symbol])
    ...
    <div className="rounded-2xl ring-1 ring-zinc-200 dark:ring-white/10 bg-white dark:bg-zinc-900 shadow-sm p-4 md:p-6 space-y-4 text-zinc-900 dark:text-zinc-100">
      ...
      <div className="flex items-center gap-2">
        <button className="px-2.5 py-1.5 rounded bg-zinc-800 text-white text-xs hover:bg-zinc-700" onClick={async()=>{
          ...
          if(next && !prevPlan){
            try{
              const { data } = await api.get('analyses', { params:{ status:'archived', trade_type: 'spot' } })
      ...
      {/* LLM SPOT II report */}
      <LLMReport analysisId={plan.id} verification={verification} onApplied={()=> onUpdate()} onPreview={setGhost} kind='spot' />
      ...
      <div className="flex gap-2">
        ...
        {/* Spot verify */}
        <button disabled={verifying || !llmEnabled || (typeof llmRemaining==='number' && llmRemaining<=0)} title={!llmEnabled? 'LLM nonaktif (limit/budget)':'Tanya GPT'}
          onClick={async()=>{
          try{
            setVerifying(true)
...
// FuturesSummary removed (moved to Futures page)
