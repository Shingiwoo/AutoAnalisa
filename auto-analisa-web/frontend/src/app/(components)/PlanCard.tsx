...
export default function PlanCard({plan, onUpdate, llmEnabled, llmRemaining, onAfterVerify}:{plan:any,onUpdate:()=>void, llmEnabled?:boolean, llmRemaining?:number, onAfterVerify?:()=>void}){
  ...
  return (
    <div className="rounded-2xl ring-1 ring-zinc-200 dark:ring-white/10 bg-white dark:bg-zinc-900 shadow-sm p-4 md:p-6 space-y-4 text-zinc-900 dark:text-zinc-100">
      ...
      <LLMVerifyBlock plan={p} verification={verification || p?.llm_verification} />
...
function DiffList({ label, oldVals, newVals, colorUp, colorDown }:{ label:string, oldVals:any[], newVals:any[]|undefined, colorUp:string, colorDown:string }){
  ...
  return (
    <div>
      ...
      <dd className="text-sm">
        {kept.length>0 && <span className="text-zinc-300">{kept.join(' · ')}</span>}
        {added.length>0 && <span className={`ml-2 ${colorUp}`}>{added.join(' · ')}</span>}
      </dd>
    </div>
  )
}

function LLMVerifyBlock({ plan, verification }:{ plan:any, verification:any }){
  ...
  return (
    <div className="rounded-xl ring-1 ring-cyan-500/20 bg-cyan-500/5 p-3 space-y-2">
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <DiffList label="Entry" oldVals={plan.entries||[]} newVals={sug.entries} colorUp="text-emerald-400" colorDown="text-rose-400" />
        <DiffList label="TP" oldVals={plan.tp||[]} newVals={sug.tp} colorUp="text-emerald-400" colorDown="text-rose-400" />
        {typeof sug.invalid!== 'undefined' && (
          <div>
            <dt className="text-zinc-500">Invalid (LLM)</dt>
            <dd className={`text-sm ${sug.invalid>plan.invalid?'text-rose-400':'text-emerald-400'}`}>{sug.invalid}</dd>
          </div>
        )}
      </dl>
    </div>
